    ...
    if(comentar.comentario === ""){
        ...
        con.query('INSERT INTO Comentarios (comentario,publicacion,usuario, fecha) VALUES (?,?,?, NOW())', comentarioArray, function(err,result){
            if(err){
                ...
                con.query('SELECT usuario FROM Publicaciones WHERE idPublicaciones = ?', comentar.idPublicacion , function(err,row,field){
                    if(err){
                        throw err;
                    }
                    else{
                        ...
                        con.query('INSERT INTO Notificaciones (usuarioInteraccion,usuarioNotificacion,interaccion, publicacion, fecha, idInteraccion) VALUES (?,?,?,?, NOW(),?)', notiArray, function(err,result){
                            if(err){
                                throw err;
                            }
                            res.send('comentario y notificacion insertado correctamente');
                        });
                    }
...
app.post('/like', function(req,res){
    ...
    con.query('SELECT * FROM Likes WHERE publicacion = ? AND usuario = ?', like.idPublicacion , function(err,filas,columnas){
        if(err){
            throw err;
        }
            ...
            con.query('INSERT INTO Likes (publicacion,usuario,fecha) VALUES (?,?,NOW())', likeArray, function(err,result){
                if(err){
                    ...
                    con.query('SELECT usuario FROM Publicaciones WHERE idPublicaciones = ?', like.idPublicacion , function(err,row,field){
                        if(err){
                            throw err;
                        }
                        else{
                            ...
                            con.query('INSERT INTO Notificaciones (usuarioInteraccion,usuarioNotificacion,interaccion, publicacion, fecha, idInteraccion) VALUES (?,?,?,?, NOW(),?)', notiArray, function(err,result){
                                ...
                                res.send('like y notificacion insertado correctamente');
                            });
                        }
